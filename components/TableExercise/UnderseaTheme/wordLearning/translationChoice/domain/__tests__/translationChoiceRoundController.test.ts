...
function createTestController(
  roundCount: number,
  overrides: Partial<Parameters<typeof createTranslationChoiceRoundController>[0]> = {},
) {
...
describe('createTranslationChoiceRoundController', () => {
  ...
  it('moves from transform to resolve on correct selection', () => {
    const { controller } = createTestController(2);
    controller.notifyEnterComplete();
  ...
  it('ignores correct selection when not in transform', () => {
    const { controller } = createTestController(2);
    controller.notifyCorrectSelection();
  ...
  it('passes through the full lifecycle for one round', () => {
    const recordedPhases: TranslationChoiceRoundPhase[] = [];
    const timerList: Array<{ fn: () => void; delayMs: number }> = [];
    const ctrl = createTranslationChoiceRoundController({
      roundCount: 2,
      scheduleTimer: (fn, delayMs) => {
  ...
  it('ignores duplicate correct-selection events', () => {
    const { controller, onPhaseChange } = createTestController(2);
    controller.notifyEnterComplete();
    controller.notifyCorrectSelection();
    controller.notifyCorrectSelection();
