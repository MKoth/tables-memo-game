import {
  ...
  rollBodyTint,
} from '../jellyfishVisualTokens';

describe('jellyfishVisualTokens', () => {
  it('uses a shared reference bell size for font scaling', () => {
    expect(REFERENCE_BODY_BELL_SIZE).toBe(55);
    expect(computeJellyfishFontScale(55)).toBe(1);
    expect(computeJellyfishFontScale(27.5)).toBeCloseTo(0.5);
  });
