import { planSentenceRowStems, STEM_ANCHOR_MARGIN, STEM_BASE_WIDTH, STEM_TOP_WIDTH } from '../planSentenceRowStems';
...
describe('planSentenceRowStems', () => {
  it('returns an empty plan for no slots', () => {
    expect(
      planSentenceRowStems({ roundPos: 0, ...SCREEN, slotCenters: [] }),
    ).toEqual([]);
  });

  it('plans one bush per slot with anchors above the screen top', () => {
    const plans = planSentenceRowStems({
      roundPos: 2,
      ...SCREEN,
    ...
    for (const bush of plans) {
      ...
      expect(stem.baseY).toBe(-STEM_ANCHOR_MARGIN);
  ...
  it('is deterministic per (roundPos, slotCenters)', () => {
    ...
    expect(planSentenceRowStems(input)).toEqual(planSentenceRowStems(input));
  });

  it('changes anchors on respawn (different roundPos)', () => {
    const a = planSentenceRowStems({ roundPos: 0, ...SCREEN, slotCenters: slotCenters(4) });
    const b = planSentenceRowStems({ roundPos: 1, ...SCREEN, slotCenters: slotCenters(4) });
  ...
  it('changes the stem count when the sentence length changes', () => {
    const a = planSentenceRowStems({ roundPos: 0, ...SCREEN, slotCenters: slotCenters(3) });
    const b = planSentenceRowStems({ roundPos: 0, ...SCREEN, slotCenters: slotCenters(7) });
