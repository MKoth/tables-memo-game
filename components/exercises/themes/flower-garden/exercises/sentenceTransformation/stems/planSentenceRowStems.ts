...
export type PlanSentenceRowStemsInput = {
  roundPos: number;
  screenWidth: number;
  screenHeight: number;
...
export const STEM_BASE_WIDTH = 3.5;
export const STEM_TOP_WIDTH = 16;
...
/**
 * Per-round stem plan for the sentence row: one bush per slot with a single
 * stem anchored above the screen top, so the stem base stays off-screen while
 * the attached rose floats below it. Re-planned per round (respawn) with a
 * deterministic seed, so counts and anchors are known before the roses float
 * in from outside.
 */
export function planSentenceRowStems(
  input: PlanSentenceRowStemsInput,
): BushConfig[] {
  ...
  const rng = createRng(
    hashSeedString(`sentence-row-stems:${roundPos}:${slotCenters.length}`),
  );
  ...
  for (let slot = 0; slot < slotCenters.length; slot++) {
    ...
    const baseX =
      ...
      screenWidth;
    const baseY = -STEM_ANCHOR_MARGIN;
