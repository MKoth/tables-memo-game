...
import { allocateSpecies, type SpeciesWeights } from './speciesAllocator';
...
export function createRandomVisualSpawn(
  rng: Rng,
  speciesWeights: SpeciesWeights = ROAMER_SPECIES_WEIGHTS,
): RoamerSpawn {
  const wingPairIndex = assignWingPairIndices(1, rng)[0]!;
  const legBasePhase = rng() * TWO_PI;
  const species = allocateSpecies(1, speciesWeights, rng)[0]!;
...
export function createRoamerSpawnsFromWords(
  ...
  const species = allocateSpecies(count, speciesWeights, rng);
