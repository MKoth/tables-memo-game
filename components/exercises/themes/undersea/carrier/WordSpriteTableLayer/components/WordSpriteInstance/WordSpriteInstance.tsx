...
function compileWordSpriteCombinedEffect(): SkRuntimeEffect {
  const effect = Skia.RuntimeEffect.Make(JELLYFISH_COMBINED_DEFORM_SKSL);
  if (!effect) {
    throw new Error('Failed to compile wordSprite combined deform shader');
  }
  return effect;
}

const wordSpriteCombinedEffect = compileWordSpriteCombinedEffect();
...
export type WordSpriteInstanceProps = {
  bellImage: SkImage;
  ...
  dynamicOverrides?: SharedValue<WordSpriteDynamicOverrides>;
  clock: SharedValue<number>;
};

export type WordSpriteDynamicOverrides = {
  tintMode: number;
  tintStrength: number;
...
export function WordSpriteInstance({
  bellImage,
  tentacleImage,
  layoutX,
  layoutY,
  ...
  return (
    <Rect x={tentacleX} y={tentacleY} width={tentacleSize} height={tentacleSize}>
      <Shader source={wordSpriteCombinedEffect} uniforms={uniforms}>
        <ImageShader
          image={tentacleImage}
          x={tentacleX}
          y={tentacleY}
