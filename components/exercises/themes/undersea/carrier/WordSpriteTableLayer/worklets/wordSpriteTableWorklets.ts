...
export { clampW } from '../layout/computeWordSpriteLayout';
...
export function tryFocusWordSprite(
  hitIdx: number,
  revertBiasX: number,
  revertBiasY: number,
...
/** Pan/zoom the table toward one cell — same motion as tapping that wordSprite. */
export function focusWordSpriteCell(
  hitIdx: number,
  cellGridColsSv: SharedValue<number[]>,
  cellGridRowsSv: SharedValue<number[]>,
  biasX: SharedValue<number>,
...
export function findWordSpriteIndexAtTap(
  tapX: number,
  tapY: number,
  bellSizes: number[],
  xs: number[],
...
export function triggerWordSpriteTintFlash(
  idx: number,
  preset: number,
