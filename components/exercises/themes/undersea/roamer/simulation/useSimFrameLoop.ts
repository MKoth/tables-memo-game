...
import {
  ...
  ROAMER_SEPARATION_MIN_DIST_SQ,
  ROAMER_SEPARATION_RADIUS,
  ROAMER_SEPARATION_STEER,
  ROAMER_SIM_STEP_MS,
...
import type { RoamerCaptureSharedState } from './captureTypes';
import {
...
export function useSimFrameLoop(
  ...
  const steerMinX = swimZone.x + swimZone.w * ROAMER_BOUNDARY_MARGIN_RATIO;
  const steerMaxX = swimZone.x + swimZone.w * (1 - ROAMER_BOUNDARY_MARGIN_RATIO);
  const steerMinY = swimZone.y + swimZone.h * ROAMER_BOUNDARY_MARGIN_RATIO;
  const steerMaxY = swimZone.y + swimZone.h * (1 - ROAMER_BOUNDARY_MARGIN_RATIO);
  const hardMinX = swimZone.x + ROAMER_FISH_BODY_INSET;
  const hardMaxX = swimZone.x + swimZone.w - ROAMER_FISH_BODY_INSET;
  const hardMinY = swimZone.y + ROAMER_FISH_BODY_INSET;
  const hardMaxY = swimZone.y + swimZone.h - ROAMER_FISH_BODY_INSET;
  const centerX = swimZone.x + swimZone.w * 0.5;
  const centerY = swimZone.y + swimZone.h * 0.5;

  const cellSize = ROAMER_SEPARATION_RADIUS;
  ...
  const onSimFrame = useCallback(
    (frameInfo: { timestamp: number }) => {
      ...
      for (let fishIndex = 0; fishIndex < fishCount; fishIndex++) {
        ...
        if (fishRuntime.state.value === ROAMER_FISH_STATE_SWIMMING) {
              ...
              for (let gx = cx - 1; gx <= cx + 1; gx++) {
                ...
                while (i !== -1) {
                  if (i !== fishIndex && i !== capturedFishIndex && !isFishEliminated(eliminated, i)) {
                    ...
                    if (distSq < ROAMER_SEPARATION_RADIUS * ROAMER_SEPARATION_RADIUS && distSq > ROAMER_SEPARATION_MIN_DIST_SQ) {
                      const dist = Math.sqrt(distSq);
                      const overlap = 1 - dist / ROAMER_SEPARATION_RADIUS;
                      const awayAngle = Math.atan2(dy, dx);
                      const str = Math.min(1, overlap * ROAMER_SEPARATION_STEER * dt);
