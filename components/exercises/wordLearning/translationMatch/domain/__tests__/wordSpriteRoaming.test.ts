import {
  WORD_SPRITE_SPEED,
  WORD_SPRITE_STATE_IDLE,
  WORD_SPRITE_STATE_SWIMMING,
...
function makeState(overrides: Partial<WordSpriteState> = {}): WordSpriteState {
  return {
    x: 200,
    y: 400,
    targetX: 300,
    targetY: 500,
    heading: 0,
    state: WORD_SPRITE_STATE_SWIMMING,
    stateTimer: 10,
    ...overrides,
  };
}
...
describe('stepWordSprite', () => {
  const zone = makeZone();

  it('moves toward target', () => {
    const state = makeState({ x: 200, y: 400, targetX: 300, targetY: 500 });
    const dt = 0.1;
    const next = stepWordSprite(state, dt, WORD_SPRITE_SPEED, zone, null, [state], 0, rng);
  ...
  it('clamps position to zone bounds', () => {
    ...
    const dt = 0.3;
    const next = stepWordSprite(state, dt, WORD_SPRITE_SPEED, zone2, null, [state], 0, rng);
  ...
  it('stays in place and preserves heading when idle', () => {
    ...
    expect(next.state).toBe(WORD_SPRITE_STATE_IDLE);
  });

  it('transitions from idle to swimming when timer expires', () => {
    const state = makeState({ state: WORD_SPRITE_STATE_IDLE, stateTimer: 0.05 });
    const dt = 0.1;
    const next = stepWordSprite(state, dt, WORD_SPRITE_SPEED, zone, null, [state], 0, rng);
    expect(next.state).toBe(WORD_SPRITE_STATE_SWIMMING);
  });

  it('nudges wordSprite away from keep-out disk when inside', () => {
    const disk: KeepOutDisk = { centerX: 200, centerY: 400, radius: 80 };
    const state = makeState({
      x: 200,
      y: 400,
  ...
  it('does not nudge when keepOutDisk is null', () => {
    ...
    const dt = 0.1;
    const next = stepWordSprite(state, dt, WORD_SPRITE_SPEED, zone, null, [state], 0, rng);
    const expectedX = 200 + Math.cos(Math.atan2(20, 20)) * WORD_SPRITE_SPEED * dt;
    const expectedY = 400 + Math.sin(Math.atan2(20, 20)) * WORD_SPRITE_SPEED * dt;
