...
export type WordSpriteRoamingLoopResult = {
...
export function useWordSpriteRoamingLoop({
  count,
  zoneWidth,
  zoneHeight,
  matchedIndicesSv,
  exitTargetsSv,
  keepOutDiskSv,
}: WordSpriteRoamingLoopParams): WordSpriteRoamingLoopResult {
  ...
  const initSharedValues = useCallback(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      txs.push(target.x);
      tys.push(target.y);
      sts.push(WORD_SPRITE_STATE_SWIMMING);
  ...
  useFrameCallback(({ timeSinceFirstFrame }) => {
    ...
    for (let i = 0; i < n; i++) {
      states.push({
        ...
        targetY: tys[i] ?? 0,
        heading: hds[i] ?? 0,
        state: sts[i] ?? WORD_SPRITE_STATE_SWIMMING,
        stateTimer: tims[i] ?? 0,
      });
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (isMatched(i) && !hasExitTarget) {
        ...
        newTxs.push(txs[i] ?? 0);
        newTys.push(tys[i] ?? 0);
        newSts.push(WORD_SPRITE_STATE_IDLE);
      ...
      if (hasExitTarget) {
        ...
        const speed = WORD_SPRITE_SPEED * EXIT_SPEED_MULTIPLIER;
        ...
        if (offScreen) {
          ...
          newTxs.push(exit.tx);
          newTys.push(exit.ty);
          newSts.push(WORD_SPRITE_STATE_SWIMMING);
          newTims.push(10);
        }
        continue;
      }
      ...
      const speed =
        WORD_SPRITE_SPEED +
        (Math.random() - 0.5) * WORD_SPRITE_SPEED_VARIANCE * 2;
      ...
      newAmps.push(next.state === WORD_SPRITE_STATE_IDLE ? 0 : 0.06);
